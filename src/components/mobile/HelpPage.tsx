      ...
      if (universityId) {
        ...
        if (universityData && universityData.length > 0) {
          ...
          if (university.emergency_contacts && Array.isArray(university.emergency_contacts)) {
            const security = (university.emergency_contacts as CmsContact[]).find((c) =>
          ...
          if (university.national_resources && Array.isArray(university.national_resources)) {
            const enabled = (university.national_resources as CmsContact[]).filter((r) => r.isEnabled !== false);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '24px' }}>
        ...
        {/* University Security — from CMS (hidden for open-access users) */}
        {!isOpenAccess && uniSecurity && (
          <EmergencyPill
            ...
            actions={
              getPhone(uniSecurity) ? (
                <PillButton
                  ...
                  bg="white"
                  color={spectra}
                  onClick={() => handleCall(getPhone(uniSecurity))}
                  bold
                />
              ) : null
            }
      ...
      {/* Partnership note for open-access users */}
      {isOpenAccess && (
        <div
          style={{
            backgroundColor: 'rgba(153,204,206,0.12)',
            borderRadius: '14px',
            padding: '16px 20px',
            marginBottom: '16px',
            display: 'flex',
        ...
        >
          ...
          <p style={{ fontSize: '13px', color: spectra, opacity: 0.55, margin: 0, lineHeight: 1.5 }}>
            University-specific support services become available when your institution partners with Mind Measure.
          </p>
        </div>
      )}

      {/* ═══ UNIVERSITY WELLBEING — accordion (CMS) — hidden for open-access ═══ */}
      {!isOpenAccess && uniWellbeing.length > 0 && (
                    ...
                    return (
                      ...
                      >
                        ...
                        {(r.hours || r.availability) && (
                          <p style={{ fontSize: '12px', color: spectra, margin: '0 0 10px', opacity: 0.3 }}>
                            {r.hours || r.availability}
                          </p>
                        )}
                ...
                {nationalServices.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        style={{ fontSize: '13px', color: spectra, margin: '0 0 10px', opacity: 0.45, lineHeight: 1.4 }}
                      >
                        {s.description || ''}
                      </p>
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
